...
#
export FABRIC_CA_SERVER_HOME="/etc/hyperledger/fabric-ca-2"
export FABRIC_CA_SERVER_CA_NAME="ica-org2"
export FABRIC_CA_SERVER_INTERMEDIATE_TLS_CERTFILES="/data/org2-ca-cert.pem"
export FABRIC_CA_SERVER_CSR_HOSTS="ica-org2"
export FABRIC_CA_SERVER_TLS_ENABLED=true
export FABRIC_CA_SERVER_DEBUG=true
BOOTSTRAP_USER_PASS="ica-org2-admin:ica-org2-adminpw"
PARENT_URL="https://rca-org2-admin:rca-org2-adminpw@rca-org2:7058"
...
# foreach all orderers
# foreach $count less than or equal number orderers
# enrollCAAdmin connect to root CA(if not intermediate) else connect inter CA
# register orderer with name by username password and type = orderer
# end foreach
# register admin with admin_name with attrs admin=true:ecert
...
# foreach peers
# foreach all peers
# if less than or equal: example peer1 will register in peer2, peer2 will register in peer1
# do register peer with type peer
# enforeach
# the admin identity has admin 
...
function registerIdentifies {
	registerOrdererIdentities
	registerPeerIdentities	
}
# foreach all orgs
# export getcacert -d -u https://$CA_HOST:7054 -M ORG_MSP_DIR
# finishMSPSetup: cp tlscacerts, cp intermediatecerts
# 
function getCACerts {
	for ORG in $ORGS; do
      initOrgVars $ORG
